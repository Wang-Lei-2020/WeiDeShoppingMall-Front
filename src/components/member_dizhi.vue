        ...
        <div class="infoPerBlock infoCommentBlock ">
          <div class="text_box">
            ...
            <ul class="y_adresslist clearfloat">
              <li class="y_addlist" v-on:click="toNewAddress" >
							<span class="y_addlistbtn">
								<s class="y_ico"></s>新增收货地址
							</span>
              </li>
                ...
                <el-col :span="10" v-for="(address, index) in addressList" :key="address.userId" :offset="index%3 === 0 ? 2 : 1">
                  <li>
                  ...
                  <p>{{ address.addressInfo }}</p>
                    ...
                    <a v-on:click="toUpdateAddress(address.id,address.receiver,address.phone,address.name,address.addressInfo)"><span  v-on:click="toUpdateAddress" class="y_adressbtn y_editbtn"></span></a>
                  <span class="y_adressbtn y_deletbtn"></span>
...
export default {
  ...
  methods: {
    ...
    toNewAddress: function () {
      console.log(this.$route.path);
      if (this.$route.path !== "/addAddress") {
        this.$router.push("/addAddress");
      }
    },
    toUpdateAddress: function (addressID,addressReceiver,addressPhone,addressName,addressAddressInfo) {
      // localStorage.setItem('TEMPaddressId',address.receiver);

      localStorage.setItem("TEMPorderID",addressID)
      localStorage.setItem("TEMPaddressReceiver",addressReceiver)
      localStorage.setItem("TEMPaddressPhone",addressPhone)
      localStorage.setItem("TEMPaddressName",addressName)
      localStorage.setItem("TEMPaddressAddressInfo",addressAddressInfo)
      this.$message({
        message :  localStorage.getItem("TEMPorderID")+"为订单ID"+localStorage.getItem("TEMPaddressReceiver")+"为收货人"+localStorage.getItem("TEMPaddressName"),
        type: 'success'
      })
      console.log(this.$route.path);

      if (this.$route.path !== "/updateAddress") {
