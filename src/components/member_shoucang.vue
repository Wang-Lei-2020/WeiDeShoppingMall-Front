        ...
        <div class="member_right">
          ...
          <div class="member_per">
            <ul class="nav panel">
              ...
              <li>
                <a v-on:click="toSecurity"><i class="icon-nav icon-ques"></i>安全中心</a>
              </li>
            </ul>
          </div>
          <div class="member_per">
...
<!--            <iframe src="/shoucang.html" scrolling="no" style="width: 100%;height: 1500px;" frameborder="0"></iframe>-->
            ...
            <!--mian html start-->
              ...
              <div class="m_content">
                ...
                <div class="zqrightinfo">
                  <div class="zqtypesh">
                    <p class="zqstate"><a href="#" class="zqon">全部宝贝</a><b>|</b><a href="#">失效</a></p>

                  </div>
                  <ul class="zqallbb">
                    <el-col :span="6" v-for="(Commodity,index) in CommodityList" :key="Commodity.userId" :offset="index%3 === 0 ? 2 : 1">
                      <li>
                        <div class="zqallimg"><img :src="Commodity.url" style="width:190px;height:150px"></div>
                        <p class="zqalltxt">{{ Commodity.commodityName }}</p>
                        <p class="zqalltxt">售卖店铺：{{ Commodity.sellerName }}</p>
                        <p class="zqallmoney">价格:{{ Commodity.price }}</p>
                        <div class="zqtwobtn">
                          <a href="##" v-on:click="addToShoppingCart(Commodity.commodityId,Commodity.sellerId)" class="zqtbtn zqclick y_btn_custom1">加购物车</a>
                          <a href="##" v-on:click="deleteCommodity( Commodity.commodityId,Commodity.sellerId)" class="zqtbtn y_btn_custom2">取消收藏</a>
                        </div>
                      </li>
                    </el-col>
...
export default {
  ...
  methods: {
    toShouCang:function(){
      this.$router.push({name:"member_shoucang",params:{isReload: 'true'}});
    },
    ...
    addToShoppingCart: function(commodityId,sellerId) {
      const _this = this;
      let formData = new FormData;
      formData.append('userId', localStorage.getItem("userId"));
      formData.append('commodityId', commodityId);
      formData.append('selectNum', 1);
    ...
    deleteCommodity: function (commodityId,sellId) {
      //print();
      localStorage.setItem("TEMPcommodityId",commodityId)
      const _this = this;
      let formData = new FormData;
      formData.append('userId', localStorage.getItem('userId'));
      formData.append('commodityId',commodityId)
    ...
    getCommodityList: function () {
      ...
      formData.append('userId',localStorage.getItem('userId'))
      this.$axios.post('/commodity/favoriteCommodityList', formData, {
        headers: {
          "Content-Type": "application/json;charset=utf-8"
        },
        withCredentials: true
...
.orange{color:#f63}
body{font:12px/1.5 "Hiragino Sans GB","Microsoft YaHei",simsun; font-size:13px; color:#211922; background-color:#fff}
...
.button3.white:hover{border-color:#c7c7c7 #c3c3c3 #bebebe; text-shadow:0 1px 0 #fdfdfd; -webkit-box-shadow:0 1px 1px #fff,inset 0 1px 0 #f3f3f3; -moz-box-shadow:0 1px 1px #ebebeb,inset 0 1px 0 #f3f3f3; box-shadow:0 1px 1px #fff,inset 0 1px 0 #f3f3f3}
...
.m_state_information .fr{width:49%; text-align:right; margin:75px 0 13px;}
@media (max-height:900px) and (max-width:1440px){
  .member_mian .m_orderiemt li{width:49%;}
  .m_orderiemt li .m_right {width: 185px;padding: 40px 0 40px 20px;}
  .m_have ol{margin-left:-2%;}
  .m_have li{width:48%; margin:0 0% 2% 2%;}
  .member_mian .m_edit .btn{bottom:-34px;top:auto;}
  .zqrightinfo .zqinfo{padding: 24px 30px;}
  .zqddinfo .zqpaymy{float:left;padding-right: 40px;}
  .zqtypesh .zqsearch{float:left; margin-top:10px;}
  .zqinfo .zqproinfo .w650{width: 528px;padding-bottom: 20px;}
  .m_state_box span:before{height: 10px;}
  .m_state_box span.active:after{height: 18px;width: 18px;bottom: -29px;}
  .zqfbtitle.zqtop30{margin: 35px 0 19px;}
}

@media (max-height:900px) and (max-width:1440px){
  .m_crumbs .fr{font-size:14px;}
  .m-mt{margin-top:30px;}
  .m_state_box, .m_state .img{width: 740px;}
  .m_state .img{background-size: 740px auto;height: 7px;}
  ...
  .member_mian .m_content h3{font-size:24px;line-height: 24px; margin-bottom:5px;}
  .y_adresslist li{padding: 15px 10px 0 20px;}
  .y_adresslist li p{font-size:12px;line-height:1.8em}
  .zqtypesh .zqstate{float:none;}
  .zqtypesh .zqsearch,.zqtypesh .zqsearch2{margin-left: 0px;display:block;float: left;}
  .zqtypesh .zqsearch input,.zqtypesh .zqsearch2 input{padding-left:10px;dispaly:block;}
  .zqwxinfo{font-size:12px;line-height: 1.8em;font-weight:normal;}
  .member_mian .m_security{font-size: 14px;margin: 0px 0 0 30px;width: 264px;}
  .member_mian .nav_left{width: 200px;}
  .member_mian .nav_left h3{line-height:40px;font-size:16px;}
  .member_mian .nav_left .m_box {
    padding: 8px 0;
    font-size: 14px;
  }
  .member_mian .nav_left a{line-height:34px;}
  .member_mian .mian_right .m_content{margin-left: 240px;}
  .zqddinfo .zqpaymy .zqbtxt{font-size:14px;color:red;}
  .m_list_bor{font-size:12px;line-height:22px;padding:5px 0 18px;
  }
  .zqtypesh{margin-top:10px;}
  .zqproinfo .zqittxt{
    width: 185px;
    font-size: 12px;
  }

  .m_state_information{padding: 10px 10px 10px 42px;}
  .m_state_information .fr{
    margin: 40px 0 10px;
  }

  .zqrightinfo .m_center{width:auto; padding:0 20px 0 10px;}
  .m_receiving{padding-left:30px;}

  .help-center .m_content{
    padding: 30px 70px 30px 70px;
  }
  .help-center .m_content_title{
    font-size: 28px;
  }
  .help-center .item_title{
    margin: 60px 0 20px;
  }
  .help-center .pay_content .pay_title{
    margin-bottom: 30px;
  }
  .help-center .pay_content .item_con{
    margin-bottom: 30px;
  }

  .help-center .buy_content .item_step{
    margin-top: 30px;
  }
  .help-center .buy_content img.mb90{
    margin-bottom: 60px;
  }
}
.member_mian .mian_right{
  width:100%;
}
.member_mian .mian_right .m_content{
  margin-left:260px;
}
.member_mian .mian_right .m_subscribe_tab{
...
.member_mian .mian_right .m_subscribe_tab th,
.member_mian .mian_right .m_subscribe_tab td{
  border-right: 1px #ededed solid;
}
.member_mian .mian_right .m_subscribe_tab tr{
  height: 70px;
  line-height: 20px;
  font-size: 14px;
}

.member_mian .mian_right .m_subscribe_tab tr:nth-child(odd){
  background: #fafafa;
}
.member_mian .mian_right .m_content{margin-left: 240px;}
.member_mian .m_content h3{
  font-weight:normal;
  font-size: 30px;
  line-height: 40px;
  color:#666;
}
.member_mian .m_content .m_edit_box{
  margin-top:84px;
  position:relative;
  zoom:1;
}
.member_mian .m_content .m_edit_box:after{
  content:".";display:block;height:0;visibility:hidden;clear:both;
}

.member_mian .m_content .m_image{
  width:160px;
  height:160px;
...
.member_mian .m_content .m_image img{
  margin-top:7px;
  border-radius:50%;
  -webkit-border-radius:50%;
  -o-border-radius:50%;
...
.member_mian .m_content .dialog_tit h3{font-weight:normal; font-size:14px; line-height:30px; color:#333;}
...
.help-center .m_content p.pl0{
  padding-left: 0;
}
.help-center .pay_content .m_content_title{
  margin-bottom: 36px;
}
.member_mian .m_content h3{font-size:24px;line-height: 24px; margin-bottom:5px;}
.member_mian .mian_right .m_content{margin-left: 240px;}
...
.zqrightinfo .m_title{position: absolute;left: 14px;top: -34px;font-size: 14px;}
.zqrightinfo .m_center{width:100%;padding: 0 48px;margin: 50px 0 25px;}
.zqrightinfo .m_center.m_borbox{border: 1px solid #e5e5e5;}
.zqrightinfo .m_center.m_borbox>.m_boxlist{border-top: 1px solid #e5e5e5;}
.zqrightinfo .m_center.m_borbox>.m_boxlist:first-child{border-top: none;}
.zqrightinfo .m_center + .m_center{margin:0 auto 25px;}
.zqrightinfo .y_btn_custom2 {background-color: #f00; border-color: #f00; color: #fff;}

.zqrightinfo{  position:relative; margin:0 auto; }
.zqrightinfo h1{ font-size:30px; line-height:52px; color:#666; font-weight:normal;}
.zqrightinfo .zqboxright{ /*border:1px solid #d9d9d9;*/}
.zqrightinfo .zqinfo{margin: 50px 0;padding: 24px 50px;color:#666;border:1px solid #d9d9d9;}
.zqrightinfo .zqinfo:first-child{margin-top: 0;}
.zqrightinfo .zqinfo.tao-zqinfo{border-color:#ff0000;}
.zqrightinfo .zqinfo.tao-zqinfo .zqinfo-tip{line-height: 1;margin-top: 32px;font-size: 16px;color: #666;}
.zqrightinfo .zqtype{font-size: 18px;color:#999;}
.zqrightinfo .zqtype .zqtype-time{font-size: 14px;}
.zqrightinfo .zqtype.red{color: red;}
.zqrightinfo .zqtype.title{color: #000;font-weight:bold;}
.zqrightinfo .zqstate .del-order, .zqrightinfo .zqtype .del-order{cursor: pointer;font-family: "iconfont-tcl";width: 15px;background-position: -154px -35px!important;display: inline-block;height: 20px; color:#666; float: left;}
.zqrightinfo .zqtype .del-order{float:right;}
.zqrightinfo .height1{ height:1px; border-bottom:1px solid #d9d9d9; margin:15px 0;}
.zqrightinfo .zqpmiaoshu{display: block;/* margin-top:7px; */height: auto;overflow: hidden;}
.zqrightinfo .zqpmiaoshu>p{height: 24px;line-height: 24px;overflow: hidden;}
.zqrightinfo .zqpmiaoshu>a>span{display: block;}
.zqrightinfo .zqpmiaoshu>a>span:nth-child(1){height: 40px;overflow: hidden;}
.zqrightinfo .zqpmiaoshu .zqpxinxi{float: left;}
.zqrightinfo .zqpmiaoshu .zqpxinxi:first-child{margin-right: 5px;}
.zqrightinfo .price-num{/* margin-top: 14px; */line-height: 24px;}
.zqrightinfo.m_bor{border:1px solid #d9d9d9; margin-top:25px;}
.zqrightinfo.m_bor .zqinfo{border:none;}
.zqrightinfo b {font-weight: normal;}

.zqrightinfo .m_service{margin-top:-12px;}
.zqrightinfo .m_service li{width:100%; float:none; border-top:1px solid #e5e5e5; border-bottom:1px solid #e5e5e5; margin:40px 0 15px; padding:30px 0 10px;}
.zqrightinfo .m_service li .zqproinfo{ border-top:1px solid #e5e5e5; margin-top:15px; padding-top:15px;}
.zqrightinfo .m_service li .zqproinfo.bor_no{border:none; padding:0px;}
.zqrightinfo-comm{
  padding-left: 30px;
}

.zqtypesh{margin: 30px auto;width:100%;position:relative;line-height:40px;overflow:hidden;zoom:1;}
.zqtypesh .zqstate{float:left;font-size:16px;overflow: hidden;zoom: 1;}
.zqtypesh .zqstate a{ color:#666; display:inline-block;}
.zqtypesh .zqstate b{ color:#d9d9d9; display:inline-block; margin:0 20px;  font-weight: lighter;}
.zqtypesh .zqsearch{display:inline-block;margin-top: 30px;/* float:right; */line-height:40px;border:none;font-size: 0px;}
.zqtypesh .zqsearch input{ border:1px solid #e5e5e5; color:#999; width:260px; height:40px; line-height:40px; *height:38px;     font-family: 'Microsoft Yahei','Arial'; *line-height:38px; border-right:0px;  padding-left:30px;  display:inline-block; margin:0;}
.zqtypesh .zqsearch .zqbtnn{ width:44px; height:40px; border:none;  display:inline-block; padding:0; margin:0;background:url(../assets/zqbtn.jpg) }
.zqtypesh .zqrecycle{position: absolute;top: 0;right: 0;}

.zqtypesh .zqsearch2{ display:inline-block; float:right; line-height:40px; border:none;}
.zqtypesh .zqsearch2 input{ border:1px solid #e5e5e5; color:#999; width:200px; height:40px; line-height:40px; *height:38px;     font-family: 'Microsoft Yahei','Arial'; *line-height:38px; border-right:0px;  padding-left:30px;  display:inline-block; margin:0;}
.zqtypesh .zqsearch2 .zqbtnn{ width:44px; height:40px; border:none;  display:inline-block; padding:0; margin:0;background:url(../assets/zqbtn.jpg); }

.zqallbb{ width:105%; position:relative; margin-top:30px; margin-left:-14px; overflow: hidden;zoom: 1;}
.zqallbb li{ width:312px; margin: 0 0 14px 14px; height:355px; border:1px solid #ccc; float:left;  }
.zqallbb li .zqallimg{ width:190px; height:190px; margin:25px auto 0;}
.zqallbb li .zqalltxt{ width:94%; color:#666; font-size:14px; line-height:24px;height:24px; overflow: hidden; text-align:center; margin:5px auto;}
.zqallbb li .zqallmoney{ width:94%; color:#ff0000; font-size:16px; line-height:24px; text-align:center; margin:5px auto;}
.zqallbb li .zqtwobtn{ width:92%;  margin:15px auto;}
.zqallbb li .zqtwobtn .zqtbtn{ width:130px; height:35px;  line-height:35px;    border:1px solid #dcdcdc;   font-size:14px; color:#333; text-align:center; display:block; float:left; margin:0 5px;}
.zqallbb li .zqtwobtn .zqclick{ border-color:#ff0000;  color:#fff;}
.zqallbb li .zqallimg{ width:190px; height:190px; margin:25px auto 0;}
.zqallbb li .zqalltxt{ width:94%; color:#666; font-size:14px; line-height:24px;height:24px; overflow: hidden; text-align:center; margin:5px auto;}
.zqallsc li .zqallimg{ width:190px; height:190px; margin:25px auto 0;}
.zqallsc li .zqalltxt{ width:94%; color:#666; font-size:14px; line-height:20px; height:40px; overflow: hidden; text-align:center; margin:5px auto;}
.y_btn_custom1 {color:#fff;background-color:#f00;border-color:#f00;}
.zqthreebtn .y_btn.y_btn_custom1:hover,.zqthreebtn .y_btn.y_btn_custom1:focus,.zqthreebtn .y_btn.y_btn_custom1:active,.zqthreebtn .y_btn.y_btn_custom1.active{color:#fff;background-color:#e50000;border-color:#e50000;}
.zqthreebtn .y_btn.y_btn_custom1{background: #f00;}
.reson-dialog .m_btn .y_btn.y_btn_custom1{margin-right:2%;}
</style>

<style src="../css/member.css" scoped>

</style>
<style src="../css/global.css" scoped>
